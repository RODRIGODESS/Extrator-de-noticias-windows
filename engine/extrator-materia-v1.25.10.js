...
function norm(v='') {
  return String(v || '').replace(/[\u200B-\u200D\u2060\uFEFF]/g,'').replace(/\u00a0/g,' ').replace(/[ \t]+/g,' ').replace(/\s*\n\s*/g,' ').trim();
}
...
function truncado(t='') { t=norm(t); return !!t && (/(?:\.\.\.|…)$/.test(t) || /\b(?:a|o|e|de|do|da|dos|das|em|no|na|nos|nas|para|por|com|sem)$/i.test(t) || /\s[A-Za-zÀ-ÿ]$/.test(t)); }
...
function palavras(t='') {
  return new Set(norm(t).toLocaleLowerCase('pt-BR').normalize('NFD').replace(/[\u0300-\u036f]/g,'').match(/[a-z0-9]{4,}/g)||[]);
}
function semelhanca(a='',b='') {
  const A=palavras(a), B=palavras(b); if (A.size<4 || B.size<4) return 0;
...
function limparTexto(m) {
  ...
  if (h==='clickpetroleoegas.com.br' || h==='www.clickpetroleoegas.com.br') {
    const out=[]; let pulando=false, n=0;
    for (const x of b) {
      if (/^ARTIGO\s+CONTINUA\s+ABAIXO$/i.test(x)) { pulando=true; n=0; continue; }
  ...
  if (h==='www.cnnbrasil.com.br' || h==='cnnbrasil.com.br') {
    b=b.filter(x=>!/^At\s+\d{1,2}:\d{2}\s+p\.m\.\s+ET\s+today\b/i.test(x) && !/^—\s*U\.S\.\s+Central\s+Command\b/i.test(x));
  }
  ...
  if (h==='www.metropoles.com' || h==='metropoles.com') {
    b=b.map(x=>x.replace(/\s+Confira\s+todas\s+aqui\.?\s*$/i,'').trim()).filter(Boolean);
  }
...
function primeiroH2(doc,titulo='') {
  ...
  for (const el of doc.querySelectorAll('h2')) {
    const t=norm(el.textContent); if (t.length<20 || t.length>700) continue;
    const n=t.toLowerCase(); if (n===nt) continue;
...
async function cabecalho(url) {
  const h=host(url);
  if (!['www1.folha.uol.com.br','noticias.r7.com','thmais.com.br','www.thmais.com.br'].includes(h)) return {};
  try {
    const cfg=base.carregarConfigProxy();
    const r=await base.fetchComRetry(url,{redirect:'follow',headers:{'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/149 Safari/537.36','Accept-Language':'pt-BR,pt;q=0.9,en;q=0.8','Accept':'text/html,application/xhtml+xml','Connection':'close'}},{timeoutMs:Math.max(5000,Number(cfg.TIMEOUT_MS)||45000),tentativas:2});
    if (!r.ok) return {};
    const doc=new JSDOM(await r.text(),{url}).window.document;
    const titulo=[...doc.querySelectorAll('h1')].map(x=>norm(x.textContent)).find(x=>x.length>=8 && x.length<=500)||'';
    return {titulo,subtitulo:primeiroH2(doc,titulo)};
  } catch (_) { return {}; }
}

async function corrigirCabecalho(m) {
  const h=host(m.url), antigo=norm(m.titulo), c=await cabecalho(m.url);

  if (h==='www1.folha.uol.com.br') {
    m.titulo=c.titulo || norm(m.titulo).replace(/^Eleição\s+2026\s*:\s*/i,'').replace(/\s+-\s+\d{2}\/\d{2}\/\d{4}\s+-\s+[^-]{2,80}\s+-\s+Folha\s*$/i,'').trim();
  }
...
module.exports={...base,VERSAO,extrairMateria,limparTexto,corrigirCabecalho,cabecalho};
